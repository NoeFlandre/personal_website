...
This blog post is a quick overview of our new publication.
...
We took a graduate conceptual modeling course from Fall 2023 for which the passing grade was of **70%**. The assessment was made on mostly **open-ended** questions (ill-structured problems; multiple valid solutions; emphasis on reasoning and construction). The evaluation set was totalling **5 quizzes** spanning **28 questions**. The coverage of our assessment includes modeling goals, disease models and their validity, Agent Based Models (ABMs), language models, design of experiments. The questions involved span multiple mastery levels (understand → create). In order to assess the current state of the art in LLMs, we leverage two models : **GPT-4o** and **Claude 3.5 Sonnet**. Both models were tested **with vs. without** access to course resources. This way our method is conditioning on the teaching material (slides provided as images; some merged due to input limits). We ensured control via identical quiz prompts for students and LLMs; instructor graded both.
